...
!   Common name:                NO2(adiabatic quartet ground state)
...
!PES1: totrmse=0.0706, errmax=1.1644, trainrmse=0.0685, validrmse=0.0941, testrmse=0.0795eV 
!PES2: totrmse=0.0881, errmax=1.2347, trainrmse=0.0880, validrmse=0.0875, testrmse=0.0903eV 
!PES3: 
!Average potential energy surface: totormse=0.0720eV
!***************************************************************************
      use nnparam
      implicit none
      integer i,j,k,ndriv
...
!-->  read NN weights and biases from matlab output
      ...
      nfile1=4
      nfile2=7
      file_path1 = trim(path)//"/NO2/weights_NO2_4Ap_PIPNN.txt"
      file_path2 = trim(path)//"/NO2/biases_NO2_4Ap_PIPNN.txt"
