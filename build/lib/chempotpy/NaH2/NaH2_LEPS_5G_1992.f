...
C   Common name:     NaH2 surface set 5G
C   Interface:       3-1V
...
C
C   Note:            This is a vectorized version of the 5G surface
...
C      at R(H-H)=1.402721684 bohr with an energy of -2.44383430e-21 hartrees.
C
C
        write(6,*) 'PREPOT called for NaH2 5g potential energy surface.'
        write(6,*) 'Development version.  Last modified 5 October 1994.'
        write(6,*)
...
c
     ...
     &     c2a/1.5d0/,c2b/1.0d0/,c2c/0.15d0/,deh2/4.7469d0/
      data cfm /1.8427829108061610d-4/

      rac2=1.d0/dsqrt(2.d0)
      cevau=1.d0/27.2113961d0

      write(6,100) dt2,ret2,betat2,y90c1,y90c2,y90c3,y90c4,y0c1,y0c2,
     &  y0c3,y0c4,ads1,ars1,abs1,cds1,crs1,cbs1,swa,swr0,swa2,swr20,
     &  c2a,c2b,c2c 
...
c
     ...
     &   +r2s*(-23.7299d0+3.91747d0*r(i,2))*dexp(-1.41350d0*r(i,2))
      ...
      e(i)=(coul1+coul2+coul3+deh2-rac2*dsqrt(w+cplg2*cplg2))*cevau-cfm
...
c     cevau=1.d0/27.2113961d0
      cevau=1.d0/27.211611d0

      write(6,100) ampn,ampp,ampe,ampr,erln,erlp,erle,erlr,
...
c
      ...
      rnah16=r(i,1)**6
      rnah36=r(i,3)**6
      ff=((r(i,1)-r(i,3))/r(i,2))**2
      fg=((rnah16-rnah36)/(rnah16+rnah36))**2
      hr=dsqrt(r(i,1)**2+r(i,2)**2+r(i,3)**2)
      fh=0.5d0*(1.d0+dtanh(0.75d0*(11.d0-hr)))
      ang=(ff-fg)*fh+fg
...
c
     ...
     &     a90g3/-1.219900d-2/,b90g3/1.002291d-2/,r90g3/1.118725d+1/
      ...
      write(6,100) y90d1,y90r1,y90b1,y90d2,y90r2,y90b2,y90swa,y90swr0,
     &  y0d1,y0r1,y0b1,y0d2,y0r2,y0b2,y0swa,y0swr0,c1s1,c2s1,c3s1,c4s1,
     &  c5s1,c6s1,c2a,c2b,c2c,a0g1,b0g1,r0g1,a0g2,b0g2,r0g2,a0g3,b0g3,
     &  r0g3,a90g1,b90g1,r90g1,a90g2,b90g2,r90g2,a90g3,b90g3,r90g3
...
c
     ...
     &   +r3s*(c4s1+c5s1*r(i,3))*dexp(c6s1*r(i,3))
      ...
      rf11=2.59329096d+0*(1.d0+dtanh(9.93116245d+0*(r(i,2)
     &     +6.85093745d+0)))
      rf12=1.00540010d-1*(1.d0+dtanh(-3.09412207d+0*(r(i,2)
     &     -1.74921175d+0)))
      rf13=6.80366735d-2*(1.d0+dtanh(4.47330630d+0*(r(i,2)
     &     -1.70178393d+0)))
      tr0=rf11+cstsq*(rf12+rf13*cstsq)
      sntsq=1.d0-cstsq
      rf21=4.52729085d+0*(1.d0+dtanh(-7.15957152d-1*(r(i,2)
     &     +5.34701010d-1)))
      rf22=9.41855078d-2*(1.d0+dtanh(-7.86831467d+0*(r(i,2)
     &     -1.90926538d+0)))
      rf23=2.84176599d-2*(1.d0+dtanh(-6.59302513d+0*(r(i,2)
     &     -6.09626510d+0)))
      rat=rf21+sntsq*(rf22+rf23*sntsq)

      al1=9.d0+dexp(0.238029900d+1*(0.147400334d+1-r(i,2)))
     &    *(-0.226331771d+0-0.217339562d+1*r(i,2)+0.954597998d+1
     &    *r(i,2)**2-0.233366749d+2*r(i,2)**3+0.247115748d+2*r(i,2)**4
     &    -0.128786102d+2*r(i,2)**5+0.275711758d+1*r(i,2)**6)
      al2=9.d0+dexp(0.135164094d+1*(-0.254404564d+0-r(i,2)))
     &    *(-0.106627055d+2-0.140112406d+2*r(i,2)-0.780660398d+1
     &    *r(i,2)**2+0.547697753d+1*r(i,2)**3-0.695875664d+1*r(i,2)**4
     &    +0.364334771d+1*r(i,2)**5-0.492505596d+0*r(i,2)**6)
      alt=twothd*(al1-al2)
      alb=onethd*al1+twothd*al2
      alhh=alb+alt*(1.5d0*cstsq-0.5d0)
      c6=505.5d0*alhh/(18.35755920d0+dsqrt(alb*0.5d0))
      vdnahh=(-c6*rnag**(-6))*(1.d0-dexp((-rnag/tr0)**7))
      cof=0.5d0*(1.d0+dtanh(rnag-r(i,2)))

      e(i)=e(i)*cevau+vdnahh*cof*rat-cfm
