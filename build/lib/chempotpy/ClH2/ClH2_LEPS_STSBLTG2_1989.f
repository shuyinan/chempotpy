...
C*****
         ...
         COMMON /V3CCOM/ CON, ALP, APRIME, BPRIME, Q2, Q4
C
...
C
      ...
      !WRITE (IPTPRT, 610) CON, APRIME, BPRIME, ALP, Q2, Q4
C
      DO  10 I = 1,3
...
C
C   Initialize the energy terms for the energy in the three asymptotic valleys
         EASYAB = D(1)
         EASYBC = D(2)
         EASYAC = D(3)
C
...
C   Recall DIST = R(1)**2 + R(3)**2 - R(2)**2
     ...
     1              Q2 * (ANG22/2.0D0)
             ...
             DEDR(1) = DEDR(1) + D3CDR1                                         
             DEDR(2) = DEDR(2) + D3CDR2
             DEDR(3) = DEDR(3) + D3CDR3
         ENDIF
C
600   FORMAT(/,1X,'*****','Potential Energy Surface',1X,'*****',
     *      //,1X,T5,'ClH2 GQQ potential energy surface',
     *      //,1X,T5,'Parameters:',
     ...
     *        T44, F10.5, T55, F10.5, T66, F10.5)
610   FORMAT(/, 2X, T5, 'Parameters for the three-center term:',
     * /,2X,T5,'J', T10,'=',T12,E13.5,T40,'ALFP', T46,'=',T48,E13.5,
     * /,2X,T5,'BETP',T10,'=',T12,E13.5,T40,'ALFT',T46,'=',T48,E13.5,
     * /,2X,T5,'Q2', T10,'=',T12,E13.5,T40,'Q4', T46,'=',T48,E13.5,
     * //,1X,'*****')
...
6000  FORMAT(/,2X,'In the ClH2 potential GQQ, T, RAD = ',1P,2E15.7,
