***********************************************************************
C   System:                     C2H6F
C   Common name:                C2H6+F
C   Number of derivatives:      1
C   Number of bodies:           9
C   Number of electronic surfaces: 1
C   Interface: potlib2001
C
C   References:: J. Espinosa-Garcia, J. C. Corchado, M. Garcia-Chamorro,
C                and C. Rangel, Phys. Chem. Chem. Phys. 2018, 20, 19860
C                (original fit)
C                J. Espinosa-Garcia, M. Garcia-Chamorro, J. C. Corchado,
C                S. Bhowmick, and Y. V. Suleimanov, Phys. Chem. Chem.
C                Phys. 2020, 22, 13790 (revised fit)
C
***********************************************************************
...
*************************************************************************
      SUBROUTINE POT
C
C   System:    C2H6 + F --> C2H5 + HF. 
C                                  Joaquin Espinosa-Garcia 2018
C
C              
...
C                       X(25) - X(27) : X, Y, Z for Hb
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
      DIMENSION COORD(27),DX(27)
...
c
c  calculate C-H-F in plane bending potential
c
       call chfbend(vchf)
c
c      en=vstr+vop+vip+vtor
       en=vstr+vop+vip+vtor+vchf
C 
...
C JEG 2019
       s3(4)=1.0d0
       s3(8)=1.0d0
C JEG 2019 No hay cambios en las k(T) !!
c
c
...
C CONST-35 
       ...
       DATA d3c1h    /  29.00400d0/
       DATA a1c1h    /   1.65300d0/
       DATA b1c1h    /   0.12500d0/
       DATA c1c1h    /   3.51404d0/
       DATA r0c2hr    /   1.08897d0/
       DATA r0c2hp    /   1.07577d0/
       DATA w5    /   1.00000d0/
       DATA w6    /   1.08897d0/
       DATA d1c2h    / 109.23000d0/
       DATA d3c2h    /  29.00400d0/
       DATA a1c2h    /   1.65300d0/
       DATA b1c2h    /   0.12500d0/
       DATA c1c2h    /   3.51404d0/
       DATA r0cc    /   1.50007d0/
       ...
       DATA a2c     /   0.94900d0/
       DATA r0hh    /   0.91660d0/
       DATA d1hh    / 141.24500d0/
       DATA d3hh    /  45.96400d0/
       DATA ahh     /   2.20570d0/
       DATA r0cb    /   1.35397d0/
       DATA d1cb    /  69.60900d0/
       DATA d3cb    /  22.50000d0/
       DATA acb     /   1.4890285d0/
       ...
       DATA fk0chf  /   0.100000d0/
       DATA chfsw   /   2.000000d0/
       DATA RCHFsp  /   2.747000d0/
       END

c+++++++++++++++++++++++++++++++++++++++++++
c
       subroutine chfbend(vchf)
c
c JCC: 15/2/2018
c Incorporacion del potencial de bending del CHF
c
      implicit double precision (a-h,o-z)
      CHARACTER*75 REF(5)
C
...
c      common /bonds/ r0ch
       ...
       dimension coschb(4),thetachb(4),dthchb(4)
c
c  initialise
c
       vchf=0.0d0
...
c  calculate force constants: fk0(i,j), f1(i)
       ...
       jchf = 1
       rcb=rc1b
       if (rc2b.lt.rc1b) then
             jchf=2
             rcb=rc2b
       endif
       arga=chfsw*(rcb-RCHFsp)
       if (arga.gt.19.d0) return
       ihchf=1
       do i = 2, 6
          if(rbh(i).lt.rbh(ihchf)) ihchf=i
       enddo
c
       i = ihchf
c
       if (jchf.eq.1) then
           coschb(i)=tc1h(i,1)*tbh(i,1)+tc1h(i,2)*tbh(i,2)
     *                       +tc1h(i,3)*tbh(i,3)
           coschb(i)=coschb(i)/rc1h(i)/rbh(i)
           thetachb(i)=acos(coschb(i))

           dthchb(i)=pi-thetachb(i)
c
       xb=q(nhb(1))
       yb=q(nhb(2))
       zb=q(nhb(3))
       ...
       zc=q(nc1(3))
           ...
           coschb(i)=tc2h(i,1)*tbh(i,1)+tc2h(i,2)*tbh(i,2)
     *                       +tc2h(i,3)*tbh(i,3)
           coschb(i)=coschb(i)/rc2h(i)/rbh(i)
           thetachb(i)=acos(coschb(i))
...
c
       vchf = FK0CHF*(0.75d0*Pi -
     -   ACos(((xb - xh)*(xc - xh) + (yb - yh)*(yc - yh) +
     -         (zb - zh)*(zc - zh))/
     -       (Sqrt((xb - xh)**2 + (yb - yh)**2 + (zb - zh)**2)*
     -         Sqrt((xc - xh)**2 + (yc - yh)**2 + (zc - zh)**2))))**4*
     -  (1 - Tanh(CHFSW*(-RCHFSP +
     -         Sqrt((-xb + xc)**2 + (-yb + yc)**2 + (-zb + zc)**2)))**2)
c
c          write (55,*) i, rcb, rbh(i), (pi-dthchb(i))*180.d0/pi,
c    *                 vchf*0.03812D0*627.51
