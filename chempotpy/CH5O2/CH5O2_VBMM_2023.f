...
c     SUBROUTINE SURF(V, COORD, DX, N3TM)
      SUBROUTINE POT
C
C   System:    CH3OH + OH --> CH2OH + H2O.
c                        --> CH3O + H2O 
c
C Como una primera prueba SOLO considero el canal 1, que es el
C mayoritario
C              Joaquin Espinosa-Garcia Junio 2022
C Esta version inlcuye el potencial de van der Waals
C
C              
...
C                       X(19) - X(21) : X, Y, Z for O 
C                       X(22) - X(24) : X, Y, Z for H(O)
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
c     DIMENSION COORD(24),DX(24)
...
C
     ...
     +               NULBL(NATOM),NFLAG(20),
     +               NASURF(ISURF+1,ISURF+1),NDER
c  include file for CH3OH+OH PES
c
c  CONST
c  
...
c Add Newh2o
c
       common /newh2o/nno, no(3), tno(3), rno,
     +        ded(3),angh2o(4),fkh2oeq,fkh2o(4),
     *        alph2o,angh2oeq,dkdr(4),dkdx(4,3),
     *        dedo(3),dedhi(3),dedho(3),
     +        ahh, d3hh, d1hh, r0hh
c
c jeg vdw
c
       common /newvdw/roh, toh(3)
c
...
c
       call opbend(vop)
c
c  jeg:calculate van der Waals term, vdw
c
       call vander(evdw)
c
...
c jeg vdw
       roh=sqrt(toh(1)*toh(1)+toh(2)*toh(2)+toh(3)*toh(3))
c
       do i=1,3
...
c
c------------------------------------------------------
c JEG 2022 subrutina para calcular el potencial de van der Waals 
...
c
c
      p1vdw=vdwc1*(vdwr/roh)**6.d0
      p2vdw=vdwc2*exp(-12.d0*(roh/vdwr))
      evdw=vdwe/0.03812d0/627.52d0*(p1vdw+p2vdw)
c
      return
      end
c
...
c CRD linea para eliminar la correccion
       A=3.0d0
c
...
c CRD 2022 INICIO
       do i = 1,3
         dot = 0.d0
         do j = 1,3
           dot = dot - tno(j)*tb1h(i,j)
         enddo
         cosine = dot / (rno*rb1h(i))
         cosine = min(1.d0, max(-1.d0, cosine))
         angh2o(i) = acos(cosine)
       enddo
      dot = 0.d0
      do j = 1,3
        dot = dot - tno(j)*tb2h(1,j)
      enddo
      cosine = dot / (rno*rb2h(1))
      cosine = min(1.d0, max(-1.d0, cosine))
      angh2o(4) = acos(cosine)
c CRD 2022 FIN
...
c CRD 2022 INICIO
       ...
       enddo
       arga= alph2o * (rb2h(1) - r0hh)
       if(arga.lt.19.0d0)then
         fkh2o(4) = fkh2oeq * (1 - tanh(arga))
       else
         fkh2o(4) = 0.0d0
       endif
c CRD 2022 FIN
...
c     data w3 /1.0000d0/
c     data w4 /1.09047d0/
      data d1c1h /106.23000d0/
      data d3c1h /37.00400d0/
      data a1c1h /1.60300d0/
      data b1c1h /0.52500d0/
      data c1c1h /0.55404d0/
      data r0c2hr /0.95897d0/
      data r0c2hp /0.96077d0/
c     data w5 /1.000d0/
c     data w6 /0.96897d0/
      data d1c2h /117.23000d0/
      data d3c2h /42.00400d0/
      data a1c2h /2.05300d0/
      data b1c2h /0.42500d0/
      ...
      data a1c /1.20997d0/
      data a2c /1.949d0/
      data r0h1h /0.97191d0/
      data d1h1h /130.458d0/
      data d3h1h /33.264d0/
      data ah1h /2.0557d0/
      data r0h2h /0.97191d0/
      data d1h2h /130.458d0/
      data d3h2h /33.264d0/
      data ah2h /2.0557d0/
      data r0c1b /1.69597d0/
      data d1c1b /45.009d0/
      data d3c1b /22.200d0/
      data ac1b /1.6390285d0/
      data r0c2b /1.85897d0/
      data d1c2b /32.609d0/
      data d3c2b /22.400d0/
      data ac2b /2.1170285d0/
      ...
      data fkh2oeq / 0.6700d0/
      data alph2o / 3.70800d0/
      data angh2oeq / 104.7132d0/
      data V3 /   0.03000d0/
      data w1 /   0.11000d0/
      data w2 /   1.08697d0/
      data w7 /   0.11000d0/
