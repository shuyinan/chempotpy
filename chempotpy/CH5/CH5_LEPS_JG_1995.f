...
C        This version corrects a bug related to the ordering of the 
C        H atoms.
...
c added 9/8/2011
c check ordering of h atoms
      call horder(cart,lswap,NATOM)
...
c
         do i=1,18
c           Roundoff in the 7th digit causes uncertainties when comparing analytic and
c           numerical derivatives. The following fix improves this situation, but a
c           better solution would be to significantly increase the precision of all of
c           the unit conversions in this code. (Jasper 7/17/11)
...
c un-reorder if we reordered earlier
       if (lswap) call unhorder(cart,natom)
       if (lswap) call unhorder(dgscart,natom)
...
C
      ...
      subroutine horder(xcall,lswap,nn)

! This interface addresses a bug in the JG pes that occurs when the last 3 CH bond vectors of the methane group do not form a right handed 
! set as anticipated by the code
! To get around this problem we merely call the code with the order of the H atoms forced to conform to a right handed set,
! reordering if needed

! xcall is an array of cartesian coordinates where the carbon is the second atom and the 4 symmetrically equivalent methane H atoms 
! are in positions 1, 3, 4, and 5.  The 6th atom is the isolated H atom that exists in the H + CH4 asymptotic limit.

         implicit none
         integer :: i,j,ii,nn
         double precision :: a(3), b(3), c(3), bxc(3), dot, xcall(nn,3),
     &              swap(3), energy
         logical lswap
         ...
         do i=1,3
         a(i)=xcall(3,i)-xcall(2,i)
         b(i)=xcall(4,i)-xcall(2,i)
         c(i)=xcall(5,i)-xcall(2,i)
         enddo

         call acrossb(b,c,bxc)
      ...
      subroutine unhorder(xcall,nn)
         integer :: i,nn
         double precision :: xcall(nn,3), swap(3)
