...
!   Common name:                PhSCH3 III (Coupled PESs, Surface set III)
...
!  BB      =  -5.913514866085083d-002
  BB      =  -6.325301976068878d-002
  alpha1  =   0.721710258187793     
!  Rw      =    1.63756083461647     
  Rw      =    1.579
!  CC      =  -3.832768992221822d-002
  CC      =  -4.099663538406905d-002
  !--- The following are unused after treating theta as tertiary;
  !---   keep them just to avoid messing up other parts
  ...
  esin = exp(-1.2533141*(R-1.82748)**2)
  W1   = BB*e1a + CC*e4a
  do i = 2, 3
     k(i) = B0(i)*exp(-alpha(i)*(R-R0(i))**2)
  end do
  ...
  !--- primary U3
  u3 = u3 + A+De*exp(-b*R) &
  !--- Yinan added term (0.0072)
       - 0.001*esin*((sin(phi))**2) &
  !--- secondary U3(R, phi)
       + W1*(1-cos(2*phi)) 
  ...
  gradu3(1) = -b*De*exp(-b*R) &
       + 2.5066282d-3*(R-1.82748)*esin*((sin(phi))**2) &
       - (2*BB*e1a+8*CC*e4a)*alpha1*(R-Rw)*(1d0-cos(2*phi)) 
  !--- (d U3)/(d phi)
  gradu3(2) = - 0.002*esin*(sin(phi))*(cos(phi)) &
              + 2*W1*sin(2*phi)
  !--- (d U3)/(d theta)
  gradu3(3) = 0d0
