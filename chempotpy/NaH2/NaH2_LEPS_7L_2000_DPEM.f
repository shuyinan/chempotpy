...
c data blocks for the long range forces
      data h2a1/2.38029900d0/,h2a2/1.47400334d0/,h2a3/-0.226331771d0/,
     &     h2a4/-2.17339562d0/,h2a5/9.54597998d0/,h2a6/-23.3366749d0/,
     &     h2a7/24.7115748d0/,h2a8/-12.8786102d0/,h2a9/2.75711758d0/
      data h2b1/1.35164094d0/,h2b2/-0.254404564d0/,h2b3/-10.6627055d0/,
     &     h2b4/-14.0112406d0/,h2b5/-7.80660398d0/,h2b6/5.47697753d0/,
     &     h2b7/-6.95875664d0/,h2b8/3.64334771d0/,h2b9/-0.492505596d0/
      data h2i1/160.484d0/,h2i2/-20.397d0/,h2i3/3.913d0/,
     &     h2i4/0.777d0/,h2i5/-7.583/,h2i6/1.501/
      data rnaie/5.139d0/,rnapol/159.267d0/,hpol/4.5d0/,hie/13.598d0/
      data hnai1/-193.341d0/,hnai2/16.968d0/,hnai3/-77.120d0/,
     &     hnai4/-1.024d0/,hnai5/6.009d0/,hnai6/-1.241d0/
      data hnau1/119.880d0/, hnau2/-0.620d0/, hnau3/31.335d0/,
     &     hnau4/70.414d0/, hnau5/-13.016d0/, hnau6/0.8681d0/
      data hnaa1/0.67d0/, hnaa2/-92.229d0/, hnaa3/51.01019d0/,
     &     hnaa4/-130.37826d0/, hnaa5/6.40285d0/
      data hnab1/0.34333d0/, hnab2/-92.229d0/, hnab3/-48.54867d0/,
     &     hnab4/-17.30523d0/, hnab5/0.99821d0/
      data balp/2.07234d0/,brho/2.0d0/, rb0/5.1998697814229d7/, 
     &     rb90/5.0d8/, f90/1.0d0/, f0/1.0d0/, 
     &     rd/9.1538750508008d8/, dalp/1.43307d0/,
     &     drho/5.71654d0/
...
c the shortest of any two distances . . .
      rsm1=r(i,3)+(r(i,2)-r(i,3))
     &  *(0.5d0+0.5d0*dtanh(0.5d0*(r(i,3)-r(i,2))))

      rsm2=r(i,3)+(r(i,1)-r(i,3))
     &  *(0.5d0+0.5d0*dtanh(0.5d0*(r(i,3)-r(i,1))))

       rsm3=r(i,1)+(r(i,2)-r(i,1))
     &  *(0.5d0+0.5d0*dtanh(0.5d0*(r(i,1)-r(i,2))))
...
c checks on angles . . .
      if (rnag.eq.0.0d0) then
        ...
        cstsq=(0.5d0*(r3s-r1s)/(r(i,2)*rnag))**2
      endif 
      ...
      if (rnag1 .eq. 0.0d0) then
         ...
         cstsq1= 0.25d0*((r3s-r2s-r1s+2.0d0*r3s*frac)/(r(i,1)*rnag1))
     &  **2
      endif
      ...
      if (rnag3 .eq. 0.0d0) then
        ...
        cstsq3= 0.25d0*((r1s-r2s-r3s+2.0d0*r1s*frac)/(r(i,3)*rnag3))
     &  **2
      endif
...
c  H2 ionization energy:
      ...
      h2ie=(h2i1+h2i2*r(i,2))*dexp(-h2i3*r(i,2))+
     &r2s*(h2i4+h2i5*r(i,2))*dexp(-h2i6*r(i,2))-s2a+hie
...
c   alb is the mean polarizability
      ...
      al1=dexp(h2a1*(h2a2-r(i,2)))*(h2a3+h2a4*r(i,2)+h2a5*r2s
     &  +h2a6*r(i,2)*r2s+h2a7*r2s*r2s+h2a8*r2s*r2s*r(i,2)
     &    +h2a9*r2s*r2s*r2s) +2.0d0*hpol

      al2=dexp(h2b1*(h2b2-r(i,2)))*(h2b3+h2b4*r(i,2)+h2b5*r2s
     &  +h2b6*r(i,2)*r2s+h2b7*r2s*r2s+h2b8*r2s*r2s*r(i,2)
...
c  The NaH + H dispersion and NaH + H induction terms must go on the lower diabat,
c  so that the adiabat is fitted correctly in the region of the coupling

c NaH properties


      rnahie1=(hnai1+hnai2*r(i,1))
     &  /(hnai3+hnai4*r(i,1)+hnai5*r1s+hnai6*r(i,1)*r1s)+rnaie

      rnahie3=(hnai1+hnai2*r(i,3))
     &  /(hnai3+hnai4*r(i,3)+hnai5*r3s+hnai6*r(i,3)*r3s)+rnaie

      rnahmu1=(hnau1*r(i,1)+hnau2*r1s)
     &  /(hnau3+hnau4*r(i,1)+hnau5*r1s+hnau6*r(i,1)*r1s)

      rnahmu3=(hnau1*r(i,3)+hnau2*r3s)
     &  /(hnau3+hnau4*r(i,3)+hnau5*r3s+hnau6*r(i,3)*r3s)

      rnahpar1=rnapol+hpol+dexp(-hnaa1*r(i,1))
     &  *(hnaa2+hnaa3*r(i,1)+hnaa4*r1s+hnaa5*r1s*r(i,1))

      rnahpar3=rnapol+hpol+dexp(-hnaa1*r(i,3))
     &  *(hnaa2+hnaa3*r(i,3)+hnaa4*r3s+hnaa5*r3s*r(i,3))

      rnahperp1=rnapol+hpol+dexp(-hnab1*r(i,1))*
     &  (hnab2+hnab3*r(i,1)+hnab4*r1s+hnab5*r1s*r(i,1))

      rnahperp3=rnapol+hpol+dexp(-hnab1*r(i,3))*
     &  (hnab2+hnab3*r(i,3)+hnab4*r3s+hnab5*r3s*r(i,3))
...
c dispersion forces
     ...
     &   ((rnaie+h2ie)*(rnagc*rnagc*rnagc+rb0))
      ...
      c6=c6*cevau*0.5d0*(1.0d0-dtanh(balp*(r(i,2)-brho)))

      c6swt1=0.5d0*(1.0d0-dtanh(dalp*(r(i,1)-drho)))
      c6swt3=0.5d0*(1.0d0-dtanh(dalp*(r(i,3)-drho)))
...
c data blocks for the long range forces
     ...
     &     h2b7/-6.95875664d0/,h2b8/3.64334771d0/,h2b9/-0.492505596d0/
      data h2i1/160.484d0/,h2i2/-20.397d0/,h2i3/3.913d0/,
     &     h2i4/0.777d0/,h2i5/-7.583/,h2i6/1.501/
      data rnaie/3.0353/,rnapol1/290.4d0/,hpol/4.5d0/,hie/13.598d0/,
     &     rnaquad/-16.8992d0/
      data hnai1/-193.341d0/,hnai2/16.968d0/,hnai3/-77.120d0/,
     &     hnai4/-1.024d0/,hnai5/6.009d0/,hnai6/-1.241d0/
      data hnau1/119.880d0/, hnau2/-0.620d0/, hnau3/31.335d0/,
     ...
     &     hnaa4/-130.37826d0/, hnaa5/6.40285d0/
      data hnab1/0.34333d0/, hnab2/-92.229d0/, hnab3/-48.54867d0/,
     &     hnab4/-17.30523d0/, hnab5/0.99821d0/
      data rqq/9.766021100d4/, rqh/2.355466062927d6/, 
     &     rb/9.3119383371714d5/, 
     &     balp/1.93701d0/, brho/1.4d0/, rd/9.1351724748364d8/, 
     &     dalp/1.43307d0/, drho/5.71654d0/
      data h2hc1/0.677d0/, h2hc2/0.378d0/, h2hc3/0.281d0/, 
     &     h2hc4/-1.022d0/, h2hc5/0.665d0/
      data h2qc1/1.532d0/, h2qc2/-.04635d0/, h2qc3/1.826d0/,
     &     h2qc4/-2.508d0/, h2qc5/1.856d0/
...
c   alb is the mean polarizability
     ...
     &     +h2a9*r2s*r2s*r2s) +2.0d0*hpol
      ...
      al2=dexp(h2b1*(h2b2-r(i,2)))*(h2b3+h2b4*r(i,2)+h2b5*r2s
     &  +h2b6*r(i,2)**3+h2b7*r2s*r2s+h2b8*r2s*r2s*r(i,2)
     &    +h2b9*r2s*r2s*r2s) +2.0d0*hpol
...
c H2 properties:
      ...
      h2hex=dexp(-h2hc1*r(i,2))
     & *(h2hc2*r(i,2)+h2hc3*r2s+h2hc4*r(i,2)*r2s+h2hc5*r2s*r2s)

      h2quad=dexp(-h2qc1*r(i,2))
     & *(h2qc2*r(i,2)+h2qc3*r2s+h2qc4*r(i,2)*r2s+h2qc5*r2s*r2s)

c NaH properties

      rnahie1=(hnai1+hnai2*r(i,1))
     &  /(hnai3+hnai4*r(i,1)+hnai5*r1s+hnai6*r(i,1)*r1s)+rnaie+una2p

      rnahie3=(hnai1+hnai2*r(i,3))
     &  /(hnai3+hnai4*r(i,3)+hnai5*r3s+hnai6*r(i,3)*r3s)+rnaie+una2p
     ...
     &  /(hnau3+hnau4*r(i,3)+hnau5*r3s+hnau6*r(i,3)*r3s)
      ...
      rnahpar1=159.267d0+hpol+dexp(-hnaa1*r(i,1))
     &  *(hnaa2+hnaa3*r(i,1)+hnaa4*r1s+hnaa5*r1s*r(i,1))

      rnahpar3=159.267d0+hpol+dexp(-hnaa1*r(i,3))
     &  *(hnaa2+hnaa3*r(i,3)+hnaa4*r3s+hnaa5*r3s*r(i,3))

      rnahperp1=159.267d0+hpol+dexp(-hnab1*r(i,1))*
     &  (hnab2+hnab3*r(i,1)+hnab4*r1s+hnab5*r1s*r(i,1))

      rnahperp3=159.267d0+hpol+dexp(-hnab1*r(i,3))*
     &  (hnab2+hnab3*r(i,3)+hnab4*r3s+hnab5*r3s*r(i,3))
...
c dispersion forces
      ...
      c6=-1.5d0*rnaie*h2ie/(rnaie+h2ie)*rnapol1*alhh/(rnagc*rnagc+rb)
