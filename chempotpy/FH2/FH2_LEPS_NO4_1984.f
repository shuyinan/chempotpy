...
C                                                                               
C   System:          FH2                                                        
C   Functional form: eLEPS (extended-London-Eyring-Polanyi-Sato) plus           
C                    a three-centered term                                      
C   Common name:     no. 4                                                      
C   Reference:       F. B. Brown, R. Steckler, D. W. Schwenke,                  
C                    D. G. Truhlar, and B. C. Garrett,                          
C                    J. Chem. Phys. 82, 188-201 (1985).                         
C                                                                               
C   Notes:           In this version of the FH2 potential the AB and AC         
C                    Sato parameters are functions of PHI, where PHI is         
C                    the angle of F-to-BC vector with the BC axis.              
C                    The BC Sato parameter is a function of R(BC), the          
C                    BC bond length, and PHI.                                   
C
...
C      LOGICAL LCOL, LDERIV, LZERO                                              
         COMMON /PRECM/ Z(3),Q(3),XJ(3),DQ(3,3),DJ(3,3),DZ(3,3),                
     +                  RR(3),A1,A2,A,B,C,D,F,G,BET,BETP,ZFH0,                  
     +                  ZHH0,ZHHST,DEG1,DEG2,DEG3,DEG4,DEG3P,                   
     +                  C1,C2,C3,C4,C5,C6,C3C,ALF,ALFP,ALFT,P3C,                
     +                  Q3C,ZHHDIF,OMP3C,OMQ3C,HA2,G2                           
C                                                                               
...
600   FORMAT (/, 2X, T5, 'PREPOT has been called for the FH2 ',                 
     *                   'potential energy surface no. 4, which',               
     *        /, 2X, T5, 'is an extended-LEPS function plus a ',                
     *                   'three-center term.',                                  
...
C                                                                               
       ...
       END DO                                                                   
C                                                                               
       REF(1)='F. B. Brown, R. Steckler, D. W. Schwenke,'                       
       REF(2)='D. G. Truhlar, B. C. Garrett,'                                   
       REF(3)='J. Chem. Phys. 82, 188(1985)'                                    
C                                                                               
      INDEXES(1) = 9                                                            
      INDEXES(2) = 1                                                            
      INDEXES(3) = 1                                                            
...
C                                                                               
C   Set the logical flag LDERIV which is the variable that actually             
C   controls the calculation of the derivatives.                                
C                                                                               
         IF (NDER .EQ. 1) THEN                                                  
             LDERIV = .TRUE.                                                    
         ELSE                                                                   
...
C                                                                               
         DEG = TDEG * PHI                                                       
         IF (.NOT.(DEG.LT.0. .OR. DEG.GT.90.D0)) GO TO 60                       
...
C                                                                               
C   PUT THE DERIVATIVES OF THE TOTAL EXCHANGE TERM IN DV                        
C                                                                               
               DO 210 J = 1,3                                                   
...
620   FORMAT(/, 2X, T5, 'Parameters for the three-center term:',                
     ...
     * /,2X,T5,'ALFP',T10,'=',T12,E13.5,T40,'ALFT',T46,'=',T48,E13.5,           
     * /,2X,T5,'P3C', T10,'=',T12,E13.5,T40,'Q3C', T46,'=',T48,E13.5)           
6000  FORMAT(/,2X,T5,'In the FH2 surface no. 4, PHI = ', F10.5,                 
     *               'degrees',                                                 
     *       /,2X,T5,'but PHI must be greater than zero.')                      
6001  FORMAT(/,2X,T5,'In the FH2 surface no. 4, PHI = ', F10.5,                 
     *               'degrees',                                                 
...
C                                                                               
     ...
     +     / 0.094D0,0.116D0,0.87D0,2.28D0,0.01056D0,                           
     +     -13.6221D0,2.70D0,0.2D0/                                             
      DATA BET,BETP,ZFH0,ZHH0,ZHHST                                             
     +     /-0.09627D0,-0.18075D0,0.175D0,                                      
     +       0.1146D0,0.167065D0/                                               
     ...
     +     /0.268340D0,0.68D0,0.078D0,2.5D0,                                    
     +      0.85D0,0.64D0/                                                      
C                                                                               
         END                                                                    
C                                                                               
